...
import com.yupi.yunpicturebackend.api.imagesearch.aliyunai.model.GetOutPaintingTaskResponse;
  ...
  //  @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    ...
    public BaseResponse<Page<Picture>> listPictureByPage(@RequestBody PictureQueryRequest pictureQueryRequest) {
        if (pictureQueryRequest == null) {
            log.error("pictureQueryRequest is null at line 155");
        }
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        if(spaceId == null) {
            //如果spaceid为空则表示要查看的是公开图库的图片
            //普通用户默认只能看到审核通过的数据
            pictureQueryRequest.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
            pictureQueryRequest.setNullSpaceId(true);
        }
        else {
            //如果要查询的是私有空间，则要获取空间权限咯
            User loginUser = userService.getLoginUser(request);
            //获取空间，判断一下空间是否存在
            Space space = spaceService.getById(spaceId);
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
            //判断请求用户是否是空间的创建者，通过id判断
